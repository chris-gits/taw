...
fn main() {
    ...
    if args.ignore_case {
        if let Some(name_pattern) = args.name {
            args.name = match Regex::new(format!("(?i){}", name_pattern.as_str()).as_str()) {
                Err(_) => {
                    fail!("Could not make name pattern case-insensitive");
                }
        ...
        if let Some(text_pattern) = args.text {
            args.text = match Regex::new(format!("(?i){}", text_pattern.as_str()).as_str()) {
                Err(_) => {
                    fail!("Could not make text pattern case-insensitive");
                }
                Ok(modified_pattern) => Some(modified_pattern),
            }
        }
    ...
    for dir_entry in walker.into_iter().flatten() {
        ...
        if args.origin.is_dir() && entry_path == args.origin {
            continue;
        }
        if !(!args.directories && !args.files) {
            if entry_path.is_file() && !args.files {
                continue;
            }
            if entry_path.is_dir() && !args.directories {
                continue;
            }
        }
        ...
        let display_path = match &args.name {
            ...
            Some(name_pattern) => {
                ...
                let entry_name = match entry_path.file_name() {
                    None => {
                        entry_warn!(entry_path, "Could not retrieve name");
                    }
                    Some(file_name_osstr) => match file_name_osstr.to_str() {
                        None => {
                            entry_warn!(entry_path, "Could not interpret name");
                        }
        ...
        let display_text_lines = match &args.text {
            ...
            Some(text_pattern) => {
                ...
                for (line_index, line) in match String::from_utf8(match read(entry_path.clone()) {
                    Err(_) => {
                        entry_warn!(entry_path, "Could not read");
                    }
                    Ok(read_bytes) => read_bytes,
                }) {
                    Err(_) => {
                        entry_warn!(entry_path, "Could not decode");
                    }
